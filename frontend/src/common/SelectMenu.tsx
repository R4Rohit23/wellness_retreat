...
export default function SelectMenu({ options, selected, setSelected }: IProps) {

  return (
    <Listbox value={selected ? selected : "Select retreat type"} onChange={setSelected}>
      ...
      <div className="relative">
        <ListboxButton className="relative w-full cursor-default rounded-md bg-white py-2 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-600 sm:text-sm sm:leading-6">
          <span className="block truncate">{selected || "Select retreat type..."}</span>
        ...
        >
          {options && options.length > 0 && options.map((person) => (
            <ListboxOption
              key={person}
              value={person}
